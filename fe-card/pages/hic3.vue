    ...
    <div>
        <div v-for="(datamurid, index) in datamurids" :key="datamurid.IdCard">
            <div class="customCardFront">
                <img   src="http://localhost:8001/kartu/static/cardtemplate/sekolah_28/front.png/"
                       width="100%"
                       height="100%"
                />
                <div>
                    <img
                        class="avatar"
                        :src="`http://localhost:8001/kartu/static/foto/sekolah_hic3/${datamurid.IdImage}.${datamurid.fotoExtension}`"
                    />
                </div>
                <div class="info">
...
<script>
 ...
 export default {
     ...
     async asyncData() {
         const { data } = await axios.get("http://localhost:8001/api/datamurid/28");
