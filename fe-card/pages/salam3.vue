    ...
    <div>
        <div v-for="(datamurid, index) in datamurids" :key="datamurid.IdCard">
            <div class="customCardFront">
                <img   src="http://localhost:8001/kartu/static/cardtemplate/sekolah_29/dpn.png/"
                       width="100%"
                       height="100%"
                />
                <div>
                    <img
                        class="avatar"
                        :src="`http://localhost:8001/kartu/static/foto/sekolah_salam3/${datamurid.IdImage}.jpg`"
                    />
                </div>
                <div class="info">
                    <p class="nama">{{datamurid.nama}}</p>
                    <p>: {{datamurid.IdImage}} / {{datamurid.other}}</p>
...
<script>
 ...
 export default {
     ...
     async asyncData() {
    const { data } = await axios.get("http://localhost:8001/api/datamurid/29");
